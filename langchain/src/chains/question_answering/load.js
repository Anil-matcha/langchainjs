...
export const loadQARefineChain = (llm,basePrompt, params = {}) => {
    const { questionPrompt = QUESTION_PROMPT_SELECTOR.getPrompt(llm), refinePrompt = REFINE_PROMPT_SELECTOR.getPrompt(llm), } = params;
    questionPrompt.promptMessages[0].prompt.template = basePrompt.concat(questionPrompt.promptMessages[0].prompt.template)
    const llmChain = new LLMChain({ prompt: questionPrompt, llm });
    const refineLLMChain = new LLMChain({ prompt: refinePrompt, llm });
    const chain = new RefineDocumentsChain({
        llmChain,
    ...
    chain.documentPrompt.template = basePrompt.concat(chain.documentPrompt.template)
    return chain;
};
